...
const createMenu = (m) => {
  ...
  const result = {
    fetchMenu: () => m,
    consumption: array,
    order: (pedido) => array.push(pedido),
    pay: () => {
      let sum = 0;
      array.forEach((item) => {
        if (Object.keys(m.food).includes(item)) {
          sum += m.food[item];
        }
        if (Object.keys(m.drink).includes(item)) {
          sum += m.drink[item];
        }
      });
...
const menu = createMenu({
  food: { coxinha: 4.5, sopa: 4.0, sashimi: 9.0 },
  drink: { agua: 2.5, refrigerante: 5.0 },
});
menu.order('coxinha');
menu.order('coxinha');
menu.order('coxinha');
