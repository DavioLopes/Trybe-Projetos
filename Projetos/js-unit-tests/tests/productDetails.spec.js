...
describe('6 - Implemente os casos de teste para a função `productDetails`', () => {
  it('Verifica se a função `productDetails` tem o comportamento esperado', () => {
    ...
    assert.notDeepEqual(productDetails('Alcool', 'mascara')[0], productDetails('Alcool', 'mascara')[1]);
    // Teste que os dois productIds terminam com 123.
    assert.strictEqual(productDetails('Alcool', 'mascara')[0].details.productId, 'Alcool123');
    assert.strictEqual(productDetails('Alcool', 'mascara')[1].details.productId, 'mascara123');
  });
});
